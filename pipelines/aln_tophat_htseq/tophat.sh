...
###### count #######
echo "Count with htseq-count"
mkdir -p count/${sample}
echo time bash ${basedir}/04-initialstats/htseq-count.sh -i align/${sample} -o count/${sample} -a "-a ${refdir}/TAIR10_gen/TAIR10_GFF3_genes.gff -t gene -i ID -s no"
time bash ${basedir}/04-initialstats/htseq-count.sh -i align/${sample} -o count/${sample} -a "-a ${refdir}/TAIR10_gen/TAIR10_GFF3_genes.gff -t gene -i ID -s no"
